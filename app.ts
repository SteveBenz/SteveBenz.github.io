...
class Greeter {
    timerToken: number;
    lastDiagnosticBlock: string;

    constructor(element: HTMLElement) {
    }

    timerCallback() {
        var diagnostics = <HTMLInputElement>document.getElementById('diagnostics');
    ...
    usbScanCodeToString(scancode: number): string {
    ...
    ps2CodeToString(scancode: number): string {
        switch (scancode) {
